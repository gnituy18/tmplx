...
func render_second_d_page(w io.Writer, state string, title string, h1 string) {
	w.Write([]byte(`<html><head>
...
var tmplxHandlers []TmplxHandler = []TmplxHandler{
	...
	},
	{
		...
		HandlerFunc: func(w http.ResponseWriter, r *http.Request) {
			...
			stateBytes, _ := json.Marshal(map[string]any{"title": title, "h1": h1})
			state := string(stateBytes)
			render_second_d_page(w, state, title, h1)
